...
static ssize_t mydrv_write(struct file* file, const char* buf, size_t count, loff_t* ppos) {
	...
	char* arr = buf;	
	if('A' <= arr[0] && arr[0] <= 'Z'){	//to lowercase
		arr[0] += 32;
	}
	else if('a' <= arr[0] && arr[0] <= 'z'){	//to upper case
		arr[0] -= 32;
	}
	
	if (copy_from_user(mydrv_data + mydrv_write_offset,arr, count))
		return -1;
    
	mydrv_write_offset += count;
	if(mydrv_write_offset == 100)
		mydrv_write_offset -=100;      //more than 100, overwrite
